...
public class DateUtil {
    ...
    public static final String PATTERN_CLASSICAL_ALL = "yyyyMMddHHmmss";
    public static final String PATTERN_CLASSICAL_SIMPLE1 = "yyyyMMdd";
     ...
     * @return
     */
    public static String[] getWeekdayStartStop() {
        if (getWeekday(new Date()) == 1) {
            Calendar cal = Calendar.getInstance();
            cal.add(Calendar.WEEK_OF_YEAR, -1);
            cal.set(Calendar.DAY_OF_WEEK, Calendar.MONDAY);
            Date date1 = cal.getTime();
            Date date2 = rollDay(date1, 6);
            String[] strs = {format(date1, PATTERN_CLASSICAL_SIMPLE), format(date2, PATTERN_CLASSICAL_SIMPLE)};
            return strs;
        }
        Calendar cal = Calendar.getInstance();
        cal.set(Calendar.DAY_OF_WEEK, Calendar.MONDAY);
     ...
     * @return
     */
    public static String[] getLastWeekdayStartStop() {
        int n = -1;
        if (getWeekday(new Date()) == 1) {
            n = -2;
        }
    ...
    public static Date dayDate2DateTime(String dateStr,String format,boolean isStartTime) {
        ...
        if( dateStr!=null && !dateStr.equals("")){
            try {
                ...
                if (isStartTime) {
                    redate = getIntegralStartTime(date);
                }else{
                    redate = getIntegralEndTime(date);
                }
            } catch (ParseException e) {
                e.printStackTrace();
            }
...
//        String[] strings = getWeekdayStartStop();
...
//        }
        ...
        try {
            ...
            System.out.println("===============" + DateUtil.dayDate2DateTime("2008-08-08","yyyy-MM-dd",true)
                    + "\n===================" + DateUtil.dayDate2DateTime("2008-08-08","yyyy-MM-dd",false));
        } catch (ParseException e) {
            e.printStackTrace();
        }
